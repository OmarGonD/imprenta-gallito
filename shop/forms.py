...
class ProfileForm(ModelForm):
    ...
    reference = forms.CharField(label='Referencia', max_length=100, required=True)

    class Meta:
        model = Profile
        fields = ('dni', 'phone_number', 'birthdate', 'gender', 'shipping_address1',
                  'reference', 'shipping_department', 'shipping_province', 'shipping_district')
...
class StepTwoForm(forms.ModelForm):
    ...
    class Meta:
        model = CartItem
        fields = ('design_file', 'comment')

    def __init__(self, *args, **kwargs):
        super(StepTwoForm, self).__init__(*args, **kwargs)
        self.fields['comment'].required = False
        self.fields['design_file'].required = False
...
class ContactForm(forms.Form):
    ...
    image = forms.ImageField(label='Adjuntar Imagen (Opcional)', required=False, widget=forms.ClearableFileInput(attrs={'class': 'block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100'}))
